...
const int imu_ch_pins[] = {27, 26};//imu channel digital pins 27 is for x vel, 26 is for y vel
Channel** imu_channels = new Channel*[imu_chs];

// Robot state stuff
double irPos[] = {-23, 170, 157, -10};     // absolute position if IR sensors, for calibrating position on startup, degrees. increasing rotates clockwise looking from the top
//^^original values were -23,170,157,-10. Changed to correct for something

int irPin[] = {9, 10, 11, 12};           // pins that ir sensors are hooked up to
...
double tOuter = 10000;                // Target length of outer (second) loop controller, microseconds
...
double aMaxYaw = 3000;                      // Max angular acceleration of yaw motor, in motor frame, degrees/second^2. Safe starting value: 5000
double wMaxYaw = 5000;                     // Max angular velocity of yaw motor, in motor frame, degrees/second. Safe starting value: 10000
...
void setup()
{
  ...
  canInit(0, CAN_BPS_500K);
  canInit(1, CAN_BPS_500K);
  ...
  attachInterrupt(channels[1]->getPin(), calcCh2, CHANGE);//calcCh2
  ...
  attachInterrupt(imu_channels[0]->getPin(), imu_calcCh1, CHANGE);//calcCh2
...
//      Serial.println();
      for (int i = 0; i<nWheels; i++){
        ...
        kinematics[i]->calc((int)v[0], (int)v[1], (int)v[2]);
      }
    } else if (mode == 1){
      //imu.updateIMU();
      //riding mode
...
//      }
  ...
  if (now - lastOuter > tOuter){
    ...
    if (channels[7]->getCh() < -400) {  //default mode is tele-op
      //Serial.print(rcLost);
      //Serial.print("   ");
    ...
    if (channels[7]->getCh() >400) {  //for riding mode
      mode = 1;
      //Serial.print("riding mode");
      //imu.printReadouts();
    }
    if (channels[7]->getCh() > -400 && channels[7]->getCh() < 400){ //when switch is in center pos, set all velocities to zero to prep for riding
      v[0] = 0;
      v[1] = 0;
      v[2] = 0;
    }
...
void calMotor() {
  ...
  delay(1000);
  double fineTune = 0.2;    // to step in less than 1 deg increments - this is the ratio (0.2 would be in 0.2 degree increments
  double angTarget = 0;
  for (int i = 0; i < (int)(325 * yRatio * 1.5 / fineTune); i++){
    checkRx();
    for (int j = 0; j < nWheels; j++){
    ...
    // Hacky override to calibration, when ir sensors are broken, manually align to forwards within 45 degrees
    /*
